...
class PixelNorm2d(nn.Module):
    ...
    def forward(self, x):
        l2_norm = torch.sqrt(torch.mean(x * x, dim=1, keepdim=True) + self.eps)    # [N, 1, S, S]
...
class VarNorm2d(nn.Module):
    ...
    def __init__(self, num_features, norm_type, *args, **kwargs):
        ...
        if norm_type == 'BN':
            self.layer = nn.BatchNorm2d(num_features, **kwargs)
        elif norm_type == 'PN':
            self.layer = PixelNorm2d(num_features, **kwargs)
        elif norm_type == 'IN':
            self.layer = nn.InstanceNorm2d(num_features, **kwargs)
        elif norm_type == 'NO':
            self.layer = None
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, norm_type, stride=1, downsample=None):
        ...
        self.bn1 = VarNorm2d(planes, norm_type)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes)
        self.bn2 = VarNorm2d(planes, norm_type)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, norm_type, stride=1, downsample=None):
        ...
        self.bn3 = VarNorm2d(planes * self.expansion, norm_type)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.bn1 = VarNorm2d(self.in_planes, self.norm_type)
        self.relu = nn.ReLU(inplace=True)
        ...
        if stride != 1 or self.in_planes != planes * block.expansion:
            if shortcut_type == 'A':
                ...
                downsample = nn.Sequential(
                    ...
                    VarNorm2d(planes * block.expansion, self.norm_type))
...
def generate_model(model_depth, **kwargs):
    ...
    if model_depth == 10:
        ...
        raise ValueError("Invalid model depth: ", model_depth, ", Choose from 10, 18, 34, 50, 101, 152, 200")
