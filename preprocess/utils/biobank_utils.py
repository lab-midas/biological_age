...
class Biobank_Dataset(object):
    ...
    def __init__(self, input_dir, cvi42_dir=None):
        ...
        tag_dir = []
        for s in subdirs:
            m = re.match('CINE_segmented_SAX_b(\w*)$', s)
            if 'INLINE' in s.upper():
                m = []
            if m:
                sax_dir += [(os.path.join(input_dir, s), int(''.join([s for s in m.group(1) if s.isdigit()])))]  # int(m.group(1)))
            ...
            for z in range(0, Z):
                ...
                for t in range(0, T):
                    ...
                    try:
                        ...
                        print('Warning: failed to read pixel_array from file {0}. '.format(os.path.join(dir[z], f)))
