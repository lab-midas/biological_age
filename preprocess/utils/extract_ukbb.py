        ...
        if len(nii_files) >= 24:
            for k in range(4):
                ...
                for f in range(1, 6):
                    ...
                    input_images.append(os.path.join(subject_dir, nii_files[k+f*4]))
                    
                input_images = [img for img in input_images if 'nobh' not in img]
...
def main():
    ...
    tool = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stitching_ubuntu1804')
